...
pub async fn store_room(db_client: &PgPool, room_id: &str) -> Result<(), StoreRoomError> {
    sqlx::query!(
        r#"
...
pub async fn next(tx: web::Data<Sender<()>>, db_client: web::Data<PgPool>) -> HttpResponse {
...
pub async fn on_handle_next_room(db_client: &PgPool) -> Result<String, anyhow::Error> {
    let room = sqlx::query!(
        r#"
            SELECT * 
            FROM rooms
...
pub async fn count_rooms(db_client: &PgPool) -> Result<i64, anyhow::Error> {
